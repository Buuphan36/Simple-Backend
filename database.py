...
class DB:
    ...
    def insert(self, query=None, table=None, attributes=None, values=None):
        if not query:
            ...
            values = tuple(values)
        result = self._execute_query(query, values, action=self.INSERT)
        if result:
            self.record_query("""INSERT INTO {} ( {} ) VALUES {}""".format(table, attributes_to_str, values))
        return result
    ...
    def update(self, query, values=None):
        if values:
            values = [x.strip(' ') for x in values]
            values = tuple(values)
            result = self._execute_query(query + '%s', values, action=self.UPDATE)
            if result:
                self.record_query(query + '{}'.format(values))
        else:
            result = self._execute_query(query, None, action=self.UPDATE)
            if result:
                self.record_query(query)
        return result

    def delete(self, query, values=None):
        result = self._execute_query(query + '%s', values, action=self.DELETE)
        if result:
            self.record_query(query + '\'{}\''.format(values))
        return result
    ...
    def trigger(self):
        scripts = ["""DROP TRIGGER IF EXISTS `user_AFTER_INSERT`""",
                   """CREATE TRIGGER `user_AFTER_INSERT` AFTER INSERT ON `user` FOR EACH ROW BEGIN INSERT INTO account (account_user_id, account_region, account_type, account_create_time, account_password) VALUES (NEW.user_id, 1, 1,'2020-02-01 01:00:00','12345'); END""",
                   """DROP TRIGGER IF EXISTS `account_AFTER_INSERT`""",
                   """CREATE TRIGGER `account_AFTER_INSERT` AFTER INSERT ON `account` FOR EACH ROW BEGIN INSERT INTO profile (account, alias) VALUES (NEW.account_id,''); END"""
                   ]
        connection = self.connect()
        cursor = connection.cursor()
